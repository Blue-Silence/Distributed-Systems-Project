...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	switch {
		...
		case rf.logs[args.PrevLog.Index].Info != args.PrevLog :
			...
			return 
		default :
			//////fmt.Println("Append  ",args.Entries, " on ID:",rf.me, "  tag:",args.Tag, "  from:",args.From)
			...
			if(rf.tailLogInfo.Index >= args.CommitIndex && rf.CommitIndex < args.CommitIndex && rf.logHistory == args.Term) {
				...
				////////fmt.Println("CommitIndex:",rf.CommitIndex,"  from:", args.From, "  on:",rf.me)
			}
...
func (rf *Raft) finalCommit() {
	...
	for {
		...
		if (rf.CommitIndex == rf.LastApplied) {
			...
			ms := 20//20 //(500 + (rand.Int63() % 300)) /20
			time.Sleep(time.Duration(ms) * time.Millisecond)
			rf.mu.Lock()
		} else {
			rf.LastApplied ++
